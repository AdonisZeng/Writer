...
class BeatPanel(ft.Column):
    def __init__(self, app):
        self.app = app
        self.list_view = ft.Column(spacing=4)
        ...
        self.empty_hint = ft.Text("（本章未拆节拍——可选模式，逐拍扩写防止中段水化）",
                                  size=11, color=ft.Colors.OUTLINE)
        super().__init__(
            controls=[
                ft.Row([ft.Text("🎬 章内节拍", size=13,
                                weight=ft.FontWeight.W_600),
                        ft.Container(expand=True)],
                       spacing=4),
                self.empty_hint,
                self.list_view,
                ft.Row([self.new_beat,
    ...
    def refresh(self, beats_json: str) -> list[dict]:
        ...
        self.empty_hint.visible = not beats
    ...
    def _build_tile(self, index: int, beat: dict) -> ft.Row:
        return ft.Row([
            ...
            ft.Text(beat.get("title", ""), size=12, expand=True,
